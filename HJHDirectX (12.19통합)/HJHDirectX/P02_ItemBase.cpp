	...
	for ( int i = 0; i < vItem.size(); ++i )
	{
		...
		if ( INPUTM->KeyDown( VK_LBUTTON ) && vItem[ i ].isPicked )
		{
			...
			for ( int j = 0; j < INVEN->getInventory().size(); ++j )
			{
				...
				{
					//INVEN->pushItem( &INVEN->getInventory()[ j ] );

					if ( INVEN->getInventory()[ j ].isItem == false )
					{
						vItem[ i ].rcPosition.x = INVEN->getInventory()[ j ].rcPosition.x;
						vItem[ i ].rcPosition.y = INVEN->getInventory()[ j ].rcPosition.y;
						INVEN->SetInventoryState( j , true );
					}
			...
			for ( int z = 0; z < QUIPMENT->getInventory().size(); ++z )
			{
				if ( PtInRect( &QUIPMENT->getInventory()[ z ].rc , ptMouse ) )
				{
					vItem[ i ].rcPosition.x = QUIPMENT->getInventory()[ z ].rc.left;
					vItem[ i ].rcPosition.y = QUIPMENT->getInventory()[ z ].rc.top;
					vItem[ i ].isPicked = false;
					select = false;
				}
			}
