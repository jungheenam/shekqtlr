...
void P02_ItemBase::SetItemInfo()
{
	...
	shose.texture[ ITEMKIND_SHOSE ] = TEXTUREM->Add( _T( "shose" ) , _T( "Texture/P02_ItemIcon/shose.png" ) , D3DX_FILTER_NONE );
...
void P02_Inventory::SetSlot()
{
	...
	for ( int i = 0; i < 16; ++i ) for ( int x = 0; x < 4; ++x ) for ( int y = 0; y < 4; ++y )
	{
		...
		slot[ i ].Texture[ SLOT_NONE ] = TEXTUREM->Add( _T( "slot" ) , _T( "Texture/P02_UI/invenSlot.png" ) , D3DX_FILTER_NONE );
